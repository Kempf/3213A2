...
uint8_t lcd_lookup(char symb)
{
    switch(symb)
    {
		...
		case 'R': return 0b01001010;
		case 'S': return 0b11001010;
...
{
    char buffer[5];
	char buffern[5]; // pls stop looking, it might break the code
    uint8_t screen = t % 9;
	if(overtime)
		t = 30;
	switch(screen)
	{
		...
		case 2:
			...
			lcd_send(1,lcd_lookup(':'));
			sprintf(buffern,"%5u",n);
			for(uint8_t c = 0; c < 5; c++)
				lcd_send(1,lcd_lookup(buffern[c]));
			lcd_send(1,lcd_lookup(' '));
			// freq
			lcd_goto(64);
