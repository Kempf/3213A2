...
uint16_t n = 0, r = 0, s = 0, h = 0, ts = 0, te = 0, count = 0, adc_array[ARR], adc_diff[DIF], zero = 0, count_diff = 0;
uint8_t adc_flag = 0, overtime = 0, th_latch = 0, oneOver = 0, toggle = 0, df_latch = 0;
int16_t  last_avg = 0;
...
int main(void)
{
	...
	while(1)
    {
        ...
        {
            adc_process(&adc_data, &th_latch, &count, adc_array, &r, &s, &h, &n, &time_ms, &ts, &te, &f, &w, &td, &zero, &toggle, adc_diff, &count_diff, &last_avg, &df_latch);
